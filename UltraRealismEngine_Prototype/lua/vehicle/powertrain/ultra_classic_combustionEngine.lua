-- Ultra Realism Engine fork. Combustion + fuel model integrated via ultra_combustionEngineIntegration.
...
local ureIntegration = rerequire("powertrain/ultra_combustionEngineIntegration")
...
local function updateTorque(device, dt)
  ...
  device.blowerAVTimer = device.blowerAVTimer + dt -- ZEIT EDIT
  if device.blowerAVTimer > 1/device.blowerRPMVarianceFrequency then -- ZEIT EDIT
    if device.blowerAVVariance > -device.blowerRPMVariance*rpmToAV then -- ZEIT EDIT
      device.blowerAVVariance = -device.blowerRPMVariance*rpmToAV -- ZEIT EDIT
    else -- ZEIT EDIT
      device.blowerAVVariance = device.blowerRPMVariance*rpmToAV -- ZEIT EDIT
    end -- ZEIT EDIT
      device.blowerAVTimer = 0 -- ZEIT EDIT
  end -- ZEIT EDIT


  local idleAV = max(device.idleAV, device.idleAVOverwrite) + device.idleRPMSmoother:get(device.idleAVVariance) + device.blowerRPMSmoother:get(device.blowerAVVariance)-- ZEIT EDIT
...
local function new(jbeamData)
  local device = {
    ...
    blowerRPMVariance = jbeamData.blowerRPMVariance or 3000, -- ZEIT EDIT
    blowerRPMVarianceFrequency = jbeamData.blowerRPMVarianceFrequency or 3, -- ZEIT EDIT
    blowerRPMSmoother = newLinearSmoothing(physicsDt*1000), -- ZEIT EDIT
    blowerAVTimer = 0, -- ZEIT EDIT
    blowerAVVariance = 0, -- ZEIT EDIT
  ...
  local rawStrokerMultCurve = {}
  if jbeamData.torqueModStrokerMult then
    local StrokerMultTorqueTable = tableFromHeaderTable(jbeamData.torqueModStrokerMult)
    local rawStrokerMultPoints = {}
    for _, v in pairs(StrokerMultTorqueTable) do
      maxAvailableRPM = max(maxAvailableRPM, v.rpm)
      table.insert(rawStrokerMultPoints, {v.rpm, v.torque})
    end
    rawStrokerMultCurve = createCurve(rawStrokerMultPoints)
  end

  local rawCylHeadMultCurve = {}
  if jbeamData.torqueModCylHeadMult then
    local CylHeadMultTorqueTable = tableFromHeaderTable(jbeamData.torqueModCylHeadMult)
    local rawCylHeadMultPoints = {}
    for _, v in pairs(CylHeadMultTorqueTable) do
      maxAvailableRPM = max(maxAvailableRPM, v.rpm)
      table.insert(rawCylHeadMultPoints, {v.rpm, v.torque})
    end
    rawCylHeadMultCurve = createCurve(rawCylHeadMultPoints)
  end

  local rawInjectorMultCurve = {}
  if jbeamData.torqueModInjectorMult then
    local InjectorMultTorqueTable = tableFromHeaderTable(jbeamData.torqueModInjectorMult)
    local rawInjectorMultPoints = {}
    for _, v in pairs(InjectorMultTorqueTable) do
      maxAvailableRPM = max(maxAvailableRPM, v.rpm)
      table.insert(rawInjectorMultPoints, {v.rpm, v.torque})
    end
    rawInjectorMultCurve = createCurve(rawInjectorMultPoints)
  end

  local rawCamshaftMultCurve = {}
  if jbeamData.torqueModCamshaftMult then
    local CamshaftMultTorqueTable = tableFromHeaderTable(jbeamData.torqueModCamshaftMult)
    local rawCamshaftMultPoints = {}
    for _, v in pairs(CamshaftMultTorqueTable) do
      maxAvailableRPM = max(maxAvailableRPM, v.rpm)
      table.insert(rawCamshaftMultPoints, {v.rpm, v.torque})
    end
    rawCamshaftMultCurve = createCurve(rawCamshaftMultPoints)
  end

  local rawFuelMultCurve = {}
  if jbeamData.torqueModFuelMult then
    local FuelMultTorqueTable = tableFromHeaderTable(jbeamData.torqueModFuelMult)
    local rawFuelMultPoints = {}
    for _, v in pairs(FuelMultTorqueTable) do
      maxAvailableRPM = max(maxAvailableRPM, v.rpm)
      table.insert(rawFuelMultPoints, {v.rpm, v.torque})
    end
    rawFuelMultCurve = createCurve(rawFuelMultPoints)
  end

  local rawIcMultCurve = {}
  if jbeamData.torqueModIcMult then
    local IcMultTorqueTable = tableFromHeaderTable(jbeamData.torqueModIcMult)
    local rawIcMultPoints = {}
    for _, v in pairs(IcMultTorqueTable) do
      maxAvailableRPM = max(maxAvailableRPM, v.rpm)
      table.insert(rawIcMultPoints, {v.rpm, v.torque})
    end
    rawIcMultCurve = createCurve(rawIcMultPoints)
  end



  local rawFilterCurve = {}
  local lastRawFilterValue = 0
  if jbeamData.torqueModFilter then
    local FilterTorqueTable = tableFromHeaderTable(jbeamData.torqueModFilter)
    local rawFilterPoints = {}
    for _, v in pairs(FilterTorqueTable) do
      maxAvailableRPM = max(maxAvailableRPM, v.rpm)
      table.insert(rawFilterPoints, {v.rpm, v.torque})
    end
    rawFilterCurve = createCurve(rawFilterPoints)
    lastRawFilterValue = rawFilterCurve[#rawFilterCurve]
  end

  local rawHeaderCurve = {}
  local lastRawHeaderValue = 0
  if jbeamData.torqueModHeader then
    local HeaderTorqueTable = tableFromHeaderTable(jbeamData.torqueModHeader)
    local rawHeaderPoints = {}
    for _, v in pairs(HeaderTorqueTable) do
      maxAvailableRPM = max(maxAvailableRPM, v.rpm)
      table.insert(rawHeaderPoints, {v.rpm, v.torque})
    end
    rawHeaderCurve = createCurve(rawHeaderPoints)
    lastRawHeaderValue = rawHeaderCurve[#rawHeaderCurve]
  end

  local rawDistributorCurve = {}
  local lastRawDistributorValue = 0
  if jbeamData.torqueModDistributor then
    local DistributorTorqueTable = tableFromHeaderTable(jbeamData.torqueModDistributor)
    local rawDistributorPoints = {}
    for _, v in pairs(DistributorTorqueTable) do
      maxAvailableRPM = max(maxAvailableRPM, v.rpm)
      table.insert(rawDistributorPoints, {v.rpm, v.torque})
    end
    rawDistributorCurve = createCurve(rawDistributorPoints)
    lastRawDistributorValue = rawDistributorCurve[#rawDistributorCurve]
  end

  local rawVelocityStackCurve = {}
  local lastRawVelocityStackValue = 0
  if jbeamData.torqueModVelocityStack then
    local VelocityStackTorqueTable = tableFromHeaderTable(jbeamData.torqueModVelocityStack)
    local rawVelocityStackPoints = {}
    for _, v in pairs(VelocityStackTorqueTable) do
      maxAvailableRPM = max(maxAvailableRPM, v.rpm)
      table.insert(rawVelocityStackPoints, {v.rpm, v.torque})
    end
    rawVelocityStackCurve = createCurve(rawVelocityStackPoints)
    lastRawVelocityStackValue = rawVelocityStackCurve[#rawVelocityStackCurve]
  end

  local rawSpacerCurve = {}
  local lastRawSpacerValue = 0
  if jbeamData.torqueModSpacer then
    local SpacerTorqueTable = tableFromHeaderTable(jbeamData.torqueModSpacer)
    local rawSpacerPoints = {}
    for _, v in pairs(SpacerTorqueTable) do
      maxAvailableRPM = max(maxAvailableRPM, v.rpm)
      table.insert(rawSpacerPoints, {v.rpm, v.torque})
    end
    rawSpacerCurve = createCurve(rawSpacerPoints)
    lastRawSpacerValue = rawSpacerCurve[#rawSpacerCurve]
  end

  local rawIntkManifoldCurve = {}
  local lastRawIntkManifoldValue = 0
  if jbeamData.torqueModIntkManifold then
    local IntkManifoldTorqueTable = tableFromHeaderTable(jbeamData.torqueModIntkManifold)
    local rawIntkManifoldPoints = {}
    for _, v in pairs(IntkManifoldTorqueTable) do
      maxAvailableRPM = max(maxAvailableRPM, v.rpm)
      table.insert(rawIntkManifoldPoints, {v.rpm, v.torque})
    end
    rawIntkManifoldCurve = createCurve(rawIntkManifoldPoints)
    lastRawIntkManifoldValue = rawIntkManifoldCurve[#rawIntkManifoldCurve]
  end

  local rawDrySumpCurve = {}
  local lastRawDrySumpValue = 0
  if jbeamData.torqueModDrySump then
    local DrySumpTorqueTable = tableFromHeaderTable(jbeamData.torqueModDrySump)
    local rawDrySumpPoints = {}
    for _, v in pairs(DrySumpTorqueTable) do
      maxAvailableRPM = max(maxAvailableRPM, v.rpm)
      table.insert(rawDrySumpPoints, {v.rpm, v.torque})
    end
    rawDrySumpCurve = createCurve(rawDrySumpPoints)
    lastRawDrySumpValue = rawDrySumpCurve[#rawDrySumpCurve]
  end
  ...
  for i = 0, maxAvailableRPM, 1 do
    ...
    local StrokerMult = rawStrokerMultCurve[i] or 1
    local CylHeadMult = rawCylHeadMultCurve[i] or 1
    local InjectorMult = rawInjectorMultCurve[i] or 1
    local CamshaftMult = rawCamshaftMultCurve[i] or 1
    local FuelMult = rawFuelMultCurve[i] or 1
    local IcMult = rawIcMultCurve[i] or 1
    local Filter = rawFilterCurve[i] or lastRawFilterValue
    local Header = rawHeaderCurve[i] or lastRawHeaderValue
    local Distributor = rawDistributorCurve[i] or lastRawDistributorValue
    local VelocityStack = rawVelocityStackCurve[i] or lastRawVelocityStackValue
    local Spacer = rawSpacerCurve[i] or lastRawSpacerValue
    local IntkManifold = rawIntkManifoldCurve[i] or lastRawIntkManifoldValue
    local DrySump = rawDrySumpCurve[i] or lastRawDrySumpValue
  ...
  local thermalsFileName = jbeamData.thermalsLuaFileName or "powertrain/classic_combustionEngineThermals"
